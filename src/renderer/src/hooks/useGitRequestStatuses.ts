...
/**
 * Loads and refreshes per-request git status for one git-backed collection.
 *
 ...
 */
export function useGitRequestStatuses(
  ...
  /**
   * Reloads request git status from the main process.
   */
  const refresh = useCallback(async (): Promise<void> => {
    ...
    try {
      const next = await window.api.gitRequestStatuses({ connectionId, collectionUuid });
      setStatuses(next);
    } catch {
      setStatuses({});
    }
  ...
  /**
   * Loads request status when the collection becomes visible.
   */
  useEffect(() => {
    if (!enabled || connectionId == null || collectionUuid == null) {
      return;
    ...
    void window.api
      .gitRequestStatuses({ connectionId, collectionUuid })
      .then((next) => {
        if (!cancelled) {
          setStatuses(next);
        }
  ...
  /**
   * Refreshes request status when the git working tree changes for this connection.
   */
  useEffect(() => {
