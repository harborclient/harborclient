...
See [Configuration](./configuration.md) and [`server.yaml.example`](https://github.com/harborclient/harborclient/blob/main/packages/team-hub/server.yaml.example) for valid sections and keys. Common VPS edits include adding an `llm:` block or changing `logging.level`.
