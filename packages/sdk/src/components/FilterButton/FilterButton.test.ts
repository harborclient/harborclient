...
describe('FilterButton', () => {
  ...
  it('renders an accessible toolbar button without an active indicator by default', () => {
    ...
    expect(button?.getAttribute('aria-label')).toBe('Filter collections');
    expect(button?.getAttribute('title')).toBe('Filter collections');
    expect(container.querySelector('.hc-status-dot')).toBeNull();
    expect(container.textContent).not.toContain('Filters applied');
  });
  ...
  it('shows the accent indicator and screen-reader label when active', () => {
    ...
    expect(container.textContent).toContain('Filters applied');
  });

  it('forwards aria-expanded and aria-haspopup for dialog anchors', () => {
    act(() => {
      root.render(
        createElement(FilterButton, {
          active: true,
