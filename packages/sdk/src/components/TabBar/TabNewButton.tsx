...
export function TabNewButton({ ariaLabel, title, wrapped, onClick }: Props): JSX.Element {
    ...
    return (
      <div className="flex shrink-0">
        <button
          type="button"
          className={cn(
            'inline-flex min-h-12 shrink-0 cursor-pointer items-center justify-center self-end rounded-t-lg border border-b-2 px-3 text-[14px]',
            tabItem(false)
          )}
          title={title}
  ...
  return (
    <div className="flex shrink-0 items-end px-1">
      <button
        type="button"
        className="hc-tab-new-button mb-2.5 inline-flex shrink-0 cursor-pointer items-center justify-center border-none bg-transparent px-3 text-[14px] text-muted hover:bg-selection hover:text-text focus-visible:bg-selection focus-visible:text-text"
        title={title}
        aria-label={ariaLabel}
        onClick={onClick}
        style={{ width: 50 }}
