...
export declare const DEFAULT_REQUEST_EDITOR_SPLIT_HEIGHT = 340;
/**
 ...
 */
export type ThemeSource = 'light' | 'dark' | 'system' | 'high-contrast' | `plugin:${string}:${string}` | `custom:${string}`;
/**
 * Request editor tab identifiers.
 */
export type EditorTab = 'params' | 'headers' | 'auth' | 'cookies' | 'body' | 'pre' | 'post' | 'comment';
/**
 ...
 */
export type SettingsSection = 'general' | 'syntax' | 'storage' | 'shortcuts' | 'proxy' | 'globals' | 'ai' | 'backup-restore' | 'git' | `plugin:${string}:${string}`;
/**
