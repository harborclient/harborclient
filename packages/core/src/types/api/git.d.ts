import type { GitCommitDetail, GitGraphLogResult, GitIdentity, GitLogEntry, GitOAuthFinishedEvent, GitRequestFileStatus, SourceControlStatus } from '../storage';
/**
 * IPC methods for git.
 */
export interface ApiGit {
     ...
     */
    gitCommit: (connectionId: string, collectionUuid: string, message: string, createHarborRoot?: boolean) => Promise<void>;
    /**
     ...
     */
    gitMergeBranch: (connectionId: string, name: string) => Promise<{
     ...
     * @param args - Git connection id and repository-relative file path.
     */
    gitOpenExternalMergeEditor: (args: {
        connectionId: string;
        filePath: string;
    }) => Promise<void>;
    /**
     ...
     */
    gitTestConnection: (connectionId: string) => Promise<{
        emptyRemote: boolean;
        canPush?: boolean;
    }>;
    /**
     ...
     */
    gitSuggestedAuthor: (connectionId?: string) => Promise<{
        name: string;
        email: string;
    }>;
    /**
    ...
    gitCommitFileDiff: (args: {
        ...
        method?: string;
        maxChars?: number;
    }) => Promise<import('../../types').GitRequestDiffFileEntry>;
    /**
     ...
     * @param collectionUuid - Stable collection uuid.
     */
    gitListItemStatuses: (connectionId: string, collectionUuid: string) => Promise<Record<string, GitRequestFileStatus>>;
    /**
     ...
     */
    gitRevertFile: (connectionId: string, collectionUuid: string, filePath: string, previousPaths?: string[]) => Promise<void>;
    /**
     ...
     */
    gitStartOAuth: (connectionId: string) => Promise<{
        userCode: string;
     ...
     */
    gitSetHostPat: (host: string, username: string, token: string, testUrl?: string, branch?: string) => Promise<{
        emptyRemote?: boolean;
        canPush?: boolean;
    }>;
    /**
     ...
     * @param branch - Optional branch name used when validating against testUrl.
     */
    gitStartHostOAuth: (host: string, testUrl?: string, branch?: string) => Promise<{
        userCode: string;
        verificationUri: string;
    }>;
    /**
     ...
     * @param branch - Optional branch name used when validating against testUrl.
     */
    gitCompleteHostOAuth: (host: string, verificationUri: string, testUrl?: string, branch?: string) => Promise<void>;
    /**
