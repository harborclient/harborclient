...
import type { PluginAssetResult, PluginEntryKind, PluginFsPickFileOptions, PluginFsSaveFileOptions, PluginGitPreview, PluginInfo, ResolvedThemeImport, SerializableMenuContribution } from '../../plugin/types';
import type { TeamHubPluginSourcesView } from '../teamHub';
...
export interface ApiPlugins {
     ...
     */
    pluginDatabaseQuery: (pluginId: string, mode: 'get' | 'all' | 'run', sql: string, params?: unknown[], txnId?: string) => Promise<unknown>;
    /**
     ...
     */
    pluginDatabaseTxEnd: (pluginId: string, txnId: string, action: 'commit' | 'rollback') => Promise<void>;
    /**
     ...
     */
    reportPluginRuntimeError: (pluginId: string, message: string | null, logDetails?: string) => Promise<PluginInfo>;
    /**
     ...
     */
    onPluginMenuCommand: (callback: (payload: {
        pluginId: string;
        command: string;
    }) => void) => () => void;
    /**
     ...
     */
    pluginFsSaveFile: (pluginId: string, content: string, options?: PluginFsSaveFileOptions) => Promise<string | null>;
    /**
     ...
     */
    executePluginAgentCommand: (pluginId: string, commandId: string, args?: unknown[]) => Promise<void>;
    /**
     * Invokes one import handler phase in a plugin agent webview.
     */
    invokePluginImportHandler: (pluginId: string, registrationId: string, phase: 'canImport' | 'import', file: {
        name: string;
        path: string;
        extension: string;
        contents: string;
     ...
     */
    onPluginsContributions: (callback: (message: {
        pluginId: string;
        op: string;
        kind?: string;
     ...
     */
    onPluginsImportHandlers: (callback: (message: {
        pluginId: string;
     ...
     */
    onPluginsHostBridge: (callback: (message: {
        pluginId: string;
        op: string;
        payload?: unknown;
    }) => void) => () => void;
     ...
     */
    onPluginsHostBridgeInvoke: (callback: (message: {
        requestId: number;
        pluginId: string;
        op: string;
        payload?: unknown;
     ...
     */
    onPluginsAgentReady: (callback: (payload: {
        pluginId: string;
    }) => void) => () => void;
    onPluginsAgentFailed: (callback: (payload: {
        pluginId: string;
        message: string;
    }) => void) => () => void;
}
