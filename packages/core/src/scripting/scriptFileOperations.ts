...
/**
 * Host-provided policy and path resolution for script filesystem operations.
 */
export interface ScriptFileOperationDependencies {
  /**
   * Resolves the confinement root for a script execution context.
   *
   * @param context - Collection connection context for the active script.
   * @returns Absolute filesystem root available to the script.
   */
  resolveRoot(context?: ScriptFileRootContext): string;
}
 ...
 */
export function resolveScriptFileRoot(
  dependencies: ScriptFileOperationDependencies,
  context?: ScriptFileRootContext
): string {
  return resolve(dependencies.resolveRoot(context));
}
...
export function executeScriptFileRequest(
  ...
  const root = resolveScriptFileRoot(dependencies, rootContext);
