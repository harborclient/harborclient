import { buildTeamHubJoinDeepLink as buildApiTeamHubJoinDeepLink, buildTeamHubJoinUrl as buildApiTeamHubJoinUrl, isInvitationCodeFormat, isTeamHubBaseUrl, normalizeTeamHubBaseUrl, parseTeamHubJoinDeepLink as parseApiTeamHubJoinDeepLink, parseTeamHubJoinUrl, summarizeInvitationAccess } from '@harborclient/team-hub-api';
/**
 * Custom URL scheme registered by HarborClient for deep links from the web.
 */
export const HARBOR_PROTOCOL = 'harborclient';
export { buildApiTeamHubJoinUrl as buildTeamHubJoinUrl, summarizeInvitationAccess };
const PLUGIN_MANIFEST_ID_PATTERN = /^[a-zA-Z][a-zA-Z0-9.-]*\.[a-zA-Z][a-zA-Z0-9.-]+$/;
const RUN_RESULT_UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
/**
 ...
 */
function parseInstallDeepLink(parsed, hostname, action) {
 ...
 */
function parseRunResultsDeepLink(parsed) {
 ...
 */
function mapInvitationLinkParams(params) {
    return {
 ...
 * @param parsed - Parsed harborclient:// URL.
 */
function parseLegacyTeamHubJoinDeepLink(parsed) {
 ...
 */
function parseTeamHubJoinDeepLink(parsed) {
 ...
 */
export function parseTeamHubInviteLink(url) {
 ...
 */
export function parseHarborDeepLink(url) {
    const trimmed = url.trim();
    if (!trimmed) {
        return null;
    }
    ...
    return (parseInstallDeepLink(parsed, 'plugin', 'install-plugin') ??
 ...
 */
export function buildPluginInstallDeepLink(pluginId) {
 ...
 * @returns Deep-link URL suitable for docs and external links.
 */
export function buildThemeInstallDeepLink(pluginId) {
 ...
 * @returns Deep-link URL suitable for docs and external links.
 */
export function buildSnippetInstallDeepLink(pluginId) {
 ...
 */
export function buildRunResultsDeepLink(uuid) {
 ...
 */
export function buildTeamHubJoinDeepLink(params) {
