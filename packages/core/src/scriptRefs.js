 ...
 */
export function createInlineScriptRef(code = '', name, stage = DEFAULT_SCRIPT_STAGE) {
    return {
 ...
 */
export function createSnippetScriptRef(snippetUuid, name, stage = DEFAULT_SCRIPT_STAGE) {
    return {
        id: crypto.randomUUID(),
        enabled: true,
        kind: 'snippet',
 ...
 */
export function copyScriptRefForClipboard(script) {
 ...
 */
export function createScriptRefFromClipboard(copied, snippets) {
 ...
 */
export function linkScriptRefToSnippet(script, snippetUuid, name) {
 ...
 */
export function normalizeScriptRefs(refs) {
 ...
 */
export function normalizeScriptRefsForCompare(refs) {
 ...
 */
export function resolveScriptRefs(refs, legacyScript) {
 ...
 */
export function readScriptRefsFromJson(raw, legacyScript) {
    if (typeof raw === 'string') {
        ...
        try {
            ...
            return resolveScriptRefs(Array.isArray(parsed) ? parsed : [], legacyScript);
        }
        catch {
            return resolveScriptRefs([], legacyScript);
        }
    ...
    if (Array.isArray(raw)) {
        return resolveScriptRefs(raw, legacyScript);
    }
    return resolveScriptRefs([], legacyScript);
}
/**
 ...
 */
export function serializeScriptRefs(refs) {
 ...
 */
export function mirrorLegacyScriptString(refs) {
 ...
 */
export function scriptRefsFromLegacyString(legacyScript) {
 ...
 */
export function ensureDefaultScriptRef(refs) {
    const normalized = normalizeScriptRefs(refs);
    if (normalized.length > 0) {
        return normalized;
    }
 ...
 */
export function resolveScriptSourceCode(script, snippets) {
 ...
 */
export function scriptAutoNameFromCode(code) {
    if (!code.trim()) {
        return null;
    }
 ...
 */
export function autoNameUnnamedScripts(refs, snippets, unnamedLabel = UNNAMED_SCRIPT_NAME) {
    return normalizeScriptRefs(refs).map((ref) => {
 ...
 */
export function mergeScriptRefsUiState(before, after) {
    ...
    const idMigrations = [];
    const consumedBeforeIds = new Set();
 ...
 */
export function hasScriptContent(refs) {
...
//# sourceMappingURL=scriptRefs.js.map